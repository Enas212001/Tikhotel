...
class NewRequestBody extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final ticketCubit = context.read<RequestCubit>();
    return RefreshIndicator(
      onRefresh: () => ticketCubit.fetchRequests(),
        ...
        slivers: [
          SliverToBoxAdapter(
            child: TopWidget(
              ...
              onChanged: (value) => ticketCubit.searchRequestsTickets(value),
            ),
          ),
                ...
                builder: (dialogContext) => GenericFilterDialog(
                  filterOptions: [
                    FilterOption(
                      ...
                      onTap: () => ticketCubit.filterRequestsTickets('all'),
                    ),
                    FilterOption(
                      value: 'new',
                      label: S.of(context).neW,
                      onTap: () => ticketCubit.filterRequestsTickets('new'),
                    ),
                    FilterOption(
                      value: 'accepted',
                      label: S.of(context).accepted,
                      onTap: () =>
                          ticketCubit.filterRequestsTickets('accepted'),
                    ),
                  ],
                ),
              ),
          ...
          BlocBuilder<RequestCubit, RequestState>(
            builder: (context, state) {
              ...
              if (state is RequestSuccess) {
                ...
                return SliverList(
                  delegate: SliverChildBuilderDelegate((context, index) {
                    return RequestInfoCard(ticket: state.tickets.data![index]);

...
class ProblemsDropDown extends StatelessWidget {
  const ProblemsDropDown({super.key, this.value, this.onChanged});
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<TicketCubit, TicketState>(
      buildWhen: (previous, current) => current is TicketDepartmentChanged,
      builder: (context, ticketState) {
        ...
        return BlocProvider(
          ...
          child: BlocBuilder<ProblemCubit, ProblemState>(
            builder: (context, problemState) {
              if (problemState is AllProblemFetched &&
                  selectedDepartment != null) {
                final problems =
                    problemState.problems.data
              ...
              } else if (problemState is AllProblemFetching) {
                return ShimmerCard(height: 40.h);
              } else if (problemState is AllProblemFetchingError) {
                return Center(child: Text(problemState.error));
              }
              return const SizedBox.shrink();
            },
          ),
